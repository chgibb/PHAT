...
app.on('ready', function () {
    ...
    try {
        fs.rename("resources/app/newCSharpCode.SharpZipLib.dll", "resources/app/ICSharpCode.SharpZipLib.dll", function (err) { });
        fs.rename("resources/app/newinstallUpdateProcess.exe", "resources/app/installUpdateProcess.exe", function (err) { });
    }
    catch (err) { }
});
...
ipc.on("getKey", function (event, arg) {
    dataMgr.pushKeyTo(arg.channel, arg.key, arg.replyChannel, event.sender);
});
